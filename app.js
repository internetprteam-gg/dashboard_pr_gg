...
async function submitRequest() {
  ...
  await postData({
    ...
    '행정전화(뒷4자리)': document.getElementById('f-tel').value.trim(),
    '신청항목': checked.join(', '),
...
function openEditRequest(id) {
  ...
  document.getElementById('edit-f-person').value = r['사업담당자'] || '';
  document.getElementById('edit-f-tel').value = r['행정전화(뒷4자리)'] || r['행정전화'] || '';
  document.getElementById('edit-f-biz-type').value = r['지속사업여부'] || '계속사업';
  document.getElementById('edit-f-reason').value = r['신청사유'] || '';
  document.getElementById('edit-f-name').value = r['사업명'] || '';
  document.getElementById('edit-f-content').value = r['사업내용'] || '';
  document.getElementById('edit-f-url').value = r['랜딩페이지'] || '';
  document.getElementById('edit-f-period').value = r['송출요청기간'] || '';

  const mediaList = parseMedia(r['신청항목'] || '');
  document.querySelectorAll('#edit-media-checks input').forEach(c => {
    c.checked = mediaList.includes(c.value);
...
async function submitEditRequest() {
  ...
  const dept = document.getElementById('edit-f-dept').value.trim();
  const person = document.getElementById('edit-f-person').value.trim();
  ...
  await postData({
    ...
    '사업부서': dept,
    '사업담당자': person,
    '행정전화(뒷4자리)': document.getElementById('edit-f-tel').value.trim(),
    '신청항목': checked.join(', '),
    '지속사업여부': document.getElementById('edit-f-biz-type').value,
    '신청사유': document.getElementById('edit-f-reason').value,
    '사업명': name,
    '사업내용': document.getElementById('edit-f-content').value.trim(),
    '랜딩페이지': document.getElementById('edit-f-url').value.trim(),
    '송출요청기간': document.getElementById('edit-f-period').value.trim(),
  });
...
function openEditComplete() {
  ...
  document.getElementById('edit-c-person').value = r['사업담당자'] || '';
  document.getElementById('edit-c-tel').value = r['행정전화(뒷4자리)'] || r['행정전화'] || '';
  document.getElementById('edit-c-name').value = r['사업명'] || '';
  document.getElementById('edit-c-url').value = r['랜딩페이지'] || '';
  document.getElementById('edit-c-m1').value = r['송출일시_홈페이지배너'] || '';
  document.getElementById('edit-c-m2').value = r['송출일시_브랜드검색광고'] || '';
  document.getElementById('edit-c-m3').value = r['송출일시_경기도뉴스레터'] || '';
  document.getElementById('edit-c-m4').value = r['송출일시_당근마켓'] || '';
  document.getElementById('edit-c-m5').value = r['송출일시_e알리미'] || '';
  document.getElementById('edit-c-m6').value = r['송출일시_경기지역화폐'] || '';

  openModal('modal-edit-complete');
}
...
async function submitEditComplete() {
  ...
  const person = document.getElementById('edit-c-person').value.trim();
